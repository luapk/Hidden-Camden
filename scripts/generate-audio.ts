 ...
 * Run: ELEVEN_API_KEY=sk_... tsx scripts/generate-audio.ts
 *
 * Outputs to public/audio/:
 *   intro.mp3, stop-01.mp3 … stop-07.mp3
 *   link-01-02.mp3 … link-06-07.mp3
 */
...
// ---------------------------------------------------------------------------
// Scripts — SFX cues stripped; paragraph breaks preserved as double newlines
// ---------------------------------------------------------------------------

const INTRO = `You're standing on the most musical half-mile on Earth. Camden should have more blue plaques than anywhere in London, but hardly any of them got put up, because the people who made this place famous were usually being barred from it at the time.
...
const STOP_01 = `Before the bands. Before the market. Before Camden was even Camden, there was a cottage on this corner, and in it lived a woman called Jinny Bingham.
...
const LINK_01_02 = `Out of the World's End, turn right, and walk up the High Street with the tube on your left. You're looking for the building that out-lasted the Luftwaffe. Forty seconds, on your left, past the shopfronts shaped like giant boots and dragons. While you walk: everything you're about to hear is true, which by Camden standards is rare.`

const STOP_02 = `In 1938, a Kerry-born builder and amateur boxer called Bill Fuller took over a rough little Irish club on this site called the Buffalo. The main entrance was round the back on Kentish Town Road. Business was steady. Space was tight.
...
// Paywall fires mid-sentence — audio cuts at "The story is..."
const LINK_02_03 = `Back the way you came, past the tube, and bear right onto Parkway. Now. In January 1979, seven young men walked into a pub on this road and told the landlord a lie. The landlord believed them, because it was a respectable lie, told politely. And that lie invented British pop music as you know it. The pub is two hundred metres ahead. The story is...`

const STOP_03 = `Not everything in Camden is loud.
...
const LINK_03_04 = `Straight up Parkway, away from the noise. Past the pet shop that's been there forever, past the blue plaques that did get put up. You're heading for a cream-coloured corner pub with hanging baskets that looks like absolutely nothing. Number 94. The most important room in British pop is the one that doesn't bother telling you.`

const STOP_04 = `Right. Stop looking at your phone and look at the pub. Cream paint, hanging baskets, looks like nothing. This is the most important room in British pop music and it holds about a hundred and fifty people.
...
const LINK_04_05 = `Out of the Castle, cross over, and head up Arlington Road. First left onto Inverness Street, the old fruit and veg market. You're about to walk into the 1990s. Specifically, into the pub where the 1990s were planned, plotted, and very nearly punched.`

const STOP_05 = `This is the pub that looks like nothing twice.
...
const LINK_05_06 = `Down Inverness Street to the High Street, turn left, and brace yourself: you're going through the market crush, under the railway bridge, towards the canal. At the bridge, bear left onto Castlehaven Road. The next pub is the one Camden nearly lost, twice. Once to fire. Once to grief.`

const STOP_06 = `Every pub on this tour has famous customers. This one had a family.
...
const LINK_06_07 = `Back to the bridge and into the market proper. Follow the crowds down towards the water, into the East Yard. You're looking for the old warehouse with a dead man's name painted on the brick. The last story is the biggest one. It's about the night everything sped up.`

const STOP_07 = `The name on this building belongs to T.E. Dingwall, a packing-case maker who stencilled his name on his warehouse and went out of business long before any of this. The name outlived the man, the trade, and very nearly the century. In 1973 the building reopened as a dance hall, kept the dead man's sign for the look of it, and accidentally made him immortal.
...
const FILES: { filename: string; text: string }[] = [
  { filename: 'intro.mp3', text: INTRO },
  { filename: 'stop-01.mp3', text: STOP_01 },
  { filename: 'link-01-02.mp3', text: LINK_01_02 },
  { filename: 'stop-02.mp3', text: STOP_02 },
  { filename: 'link-02-03.mp3', text: LINK_02_03 },
  { filename: 'stop-03.mp3', text: STOP_03 },
  { filename: 'link-03-04.mp3', text: LINK_03_04 },
  { filename: 'stop-04.mp3', text: STOP_04 },
  { filename: 'link-04-05.mp3', text: LINK_04_05 },
  { filename: 'stop-05.mp3', text: STOP_05 },
  { filename: 'link-05-06.mp3', text: LINK_05_06 },
  { filename: 'stop-06.mp3', text: STOP_06 },
  { filename: 'link-06-07.mp3', text: LINK_06_07 },
  { filename: 'stop-07.mp3', text: STOP_07 },
]
...
async function main() {
  ...
  console.log(`Generating ${FILES.length} audio files → public/audio/\n`)
